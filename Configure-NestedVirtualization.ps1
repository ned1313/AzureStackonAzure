...
$logfile = ".\EnableNestedVirtualization_$(Get-date -f yyyy-MM-dd-hh-mm-ss).txt"
...
Write-Log -logFile $logFile "Getting system information..."
...
Write-Log -logFile $logFile "done."

# grab build info out of registry
Write-Log -logFile $logFile "Getting build information..."
...
Write-Log -logFile $logFile "Validating host information..." 
...
# Write-Log -logFile $logFile "`n`nSilent Error Expected Here which verifies that computer does not support Secure Boot or is a BIOS (non-UEFI) computer:"
...
Write-Log -logFile $logFile ("The virtualization host " + $HostNested.Computer + " supports nested virtualization: ") 
if ($HostNested.HostNestedSupport) {
    Write-Log -logFile $logFile "YES" -ForegroundColor White -BackgroundColor Green
    } else {
    Write-Log -logFile $logFile "NO" -ForegroundColor White -BackgroundColor RED
    Write-Log -logFile $logFile "The following host configuration errors have been detected:"
    $HostCfgErrors
    }

# dump the details
Write-Log -logFile $logFile $HostNested
...
if($HostNested.HostNestedSupport) {
    ...
    Write-Log -logFile $logFile "`n`n=================================================================================`n`n"
    if($hyperv.State -eq "Disabled"){
        Write-Log -logFile $logFile "Hyper-V is not Enabled. Installing and Enabling Hyper-V..."
        Install-WindowsFeature -Name Hyper-V -IncludeManagementTools -Restart
        Restart-Computer
        } else {
        Write-Log -logFile $logFile "Hyper-V is already Installed and Enabled. (Skipping Hyper-V installation and Restart)`n"
        }

    Write-Log -logFile $logFile "Spinning up new instance of VM. If VM of the specified name already exists, then we will Skip Creating VM Step."
    #Spins up VM
...
if([string]::IsNullOrEmpty($vmName)) {
    Write-Log -logFile $logFile "No VM name passed"
    Exit;
}
...
Write-Log -logFile $logFile "This script will set the following for $vmName in order to enable nesting:"
    
$prompt = $false;

# Output text for proposed actions
if ($vmInfo.State -eq 'Saved') {
    Write-Log -logFile $logFile "\tSaved state will be removed"
    $prompt = $true
}
if ($vmInfo.State -ne 'Off' -or $vmInfo.State -eq 'Saved') {
    Write-Log -logFile $logFile "Vm State:" $vmInfo.State
    Write-Log -logFile $logFile "    $vmName will be turned off"
    $prompt = $true         
}
if ($vmInfo.ExposeVirtualizationExtensions -eq $false) {
    Write-Log -logFile $logFile "    Virtualization extensions will be enabled"
    $prompt = $true
}
if ($vmInfo.DynamicMemoryEnabled -eq $true) {
    Write-Log -logFile $logFile "    Dynamic memory will be disabled"
    $prompt = $true
}
if(-not $prompt) {
    Write-Log -logFile $logFile "    None, vm is already setup for nesting"
    Exit;
}
...
Write-Log -logFile $logFile "Looking for VMs..." 
...
$vms = [object[]] (Get-VM)
Write-Log -logFile $logFile ("found " + $vms.Count + " VMs.");
Write-Log -logFile $logFile "Validating virtual machines..." 
...
foreach ($vmInfo in $vmInfoList) {
    Write-Log -logFile $logFile
    Write-Log -logFile $logFile ("The virtual machine " + $vmInfo.Name + " supports nested virtualization: ")
    if ($vmInfo.SupportsNesting) {
        Write-Log -logFile $logFile "YES" 
        } else {
        Write-Log -logFile $logFile "NO"
        Write-Log -logFile $logFile "The following VM configuration errors have been detected:"
        $VmCfgErrors
        Write-Log -logFile $logFile
        }
...
Write-Log -logFile $logFile "========================================================="
Write-Log -logFile $logFile "The Bootstrap Powershell Script has Executed Successfully."
Write-Log -logFile $logFile "Next Steps: (1) Go to Hyper-V-Manager install OS onto the existing VHD or point to your custom VHD"
Write-Log -logFile $logFile "(2 - Optional if you need internet) Setup NAT Network and DCHP Server"
Write-Log -logFile $logFile "Thank you for using this forked custom-made bootstrap script for Windows Server 2016."
Write-Log -logFile $logFile "Team: AZURE COMPUTER SRE   AUTHOR: CHARLES DING"
...
Function Write-Log {
    ...
    Add-Content -Path $logFile -Value ("$(get-date -f s) $output") -PassThru | Write-Verbose
}